...
from website.transactions import increase_bid, cut_balance, remaining_time
...
def bid_page(request, auction_id):
    ...
    try:
        ...
        if request.session['username']:
            ...
            current_cost = auction.base_price
        ...
        if request.session['username']:
            ...
            if request.method == 'POST':
                ...
                if form.is_valid():
                    ...
                    if userDetails.balance > price and price > auction.base_price :
                        bid = Bid.objects.filter(user_id=user.id)
                        if not bid:
                            cut_balance(user, auction, price)
                        else:
                #     current_winner = User.objects.filter(id=latest_bid[0].user_id.id)
                #     if current_winner[0].id != user.id:
                            cut_balance(user, auction, price - auction.base_price)
                        increase_bid(user, auction, price)
